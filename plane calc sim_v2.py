...
═══════════════════════════════════════════════════════════════
PILATUS PC-21 (Military)
═══════════════════════════════════════════════════════════════

PERFORMANCE:

• Airspeed: 90–185 m/s
• Weight: ~38,000–42,000 N
• Thrust: ~12,000+ N
• Max Throttle: 1600 HP
...
The Diamond DA40 NG is an Austrian composite-body aircraft with
better fuel efficiency and modern avionics than the Cessna 172.
Its carbon fiber construction makes it lighter and more
aerodynamic.

The Pilatus PC-21 is a Swiss turboprop trainer with advanced
avionics and pressurization. It's designed to bridge the gap
between basic trainers and high-performance jet aircraft, with
a climb rate exceeding 4,000 ft/min.
 """
...
AIRCRAFT_LIMITS = {
	'civ': {
		'airspeed':(30, 85), #m/s
		'wing_area': (13.5, 16.20), #m²
		'altitude': (1350, 5000), #m
		'lift_coefficient': (0.2, 1.5),
		'drag_coefficient': (0.02, 0.08),
		'acft_weight': (10000, 11500), #N
		'wingspan': (11.00, 11.63) #m
	},

	'mil': {
		'airspeed': (70, 185), #m/s
		'wing_area': (16.28, 23.5), #m
		'altitude': (150, 11580), #m
		'lift_coefficient': (0.03, 1.6),
		'drag_coefficient': (0.03, 0.10),
		'acft_weight': (38000, 42000), #N
		'wingspan': (9.11, 10.19) #m
	}
}
...
wingspan_var = tk.DoubleVar()
labeled_entry(static_frame, 'Wingspan (m)', wingspan_var)

def warn(text):
    warn_terminal.insert('end', text + '\n')
    warn_terminal.see('end')
...
def compute_flight():
    try:
        ...
        V = airspeed_var.get()
        h = altitude_var.get()
        throttle_pct = throttle_var.get() / 100.0
        alpha_deg = aoa_var.get()
        S = wing_area_var.get()
        W = weight_var.get()
        wingspan = wingspan_var.get()
        sweep_deg = sweep_var.get()

        # Basic range checks
        if not (limits['airspeed'][0] <= V <= limits['airspeed'][1]):
            warn("Airspeed outside aircraft limits!")
            return

        if not (limits['wing_area'][0] <= S <= limits['wing_area'][1] + 2):
            warn("Wing area looks unrealistic for this class")

        if W < 5000:
            warn("Weight seems way too low")
            return
        ...
        # Air density
        if h < 11000:
            rho = rho_0 * (1 - L * h / T_0) ** (g * M / (R * L))
        else:
            rho = rho_0 * (1 - L * 11000 / T_0) ** (g * M / (R * L)) * \
                  math.exp(-g * M * (h - 11000) / (R * (T_0 - L*11000)))
        ...
        CD_min = limits['drag_coefficient'][0]

        CL = min(CL_max, 0.1 * alpha_deg)
        CD = CD_min + 0.02 * CL**2

        Lift   = 0.5 * rho * V**2 * S * CL
        Drag   = 0.5 * rho * V**2 * S * CD

        n      = Lift / W if W > 0 else 0
        L_D    = CL / CD if CD > 0 else 0

        AR     = (wingspan ** 2) / S if S > 0 else 0
        CL_alpha = (2 * math.pi * AR) / (2 + math.sqrt(4 + AR**2 * (1 + math.tan(sweep_rad)**2)))
        ...
        Thrust_available = T_max * throttle_pct

        # ── GUI variables ───────────────────────────────────
        rho_var.set(f"{rho:.4f}")
        cl_var.set(f"{CL:.2f}")
        cd_var.set(f"{CD:.3f}")
        lift_var.set(f"{Lift:,.0f}")
        drag_var.set(f"{Drag:,.0f}")
        load_var.set(f"{n:.2f}")
        l2d_var.set(f"{L_D:.1f}")
        lift_slope_var.set(f"{CL_alpha:.2f}")
        thrust_var.set(f"{Thrust_available:,.0f} N")
        ...
        if alpha_deg >= alpha_stall:
            condition = "STALLING"
            warn("STALL WARNING\nIMMEDIATE DANGER")
            warn(f"AoA {alpha_deg:.1f}° ≥ stall angle {alpha_stall}°")
            ...
            warn("INSUFFICIENT LIFT - AIRCRAFT CANNOT STAY AIRBORNE")
            warn(f"Lift {Lift:,.0f} N  <  Weight {W:,.0f} N")
        else:
        ...
        # ── Print to terminal ──────────────────────────────────────
        term_print("══════════════════════════════════════")
        term_print(f"Aircraft class:  {'Civilian' if ac=='civ' else 'Military'}")
        term_print(f"Condition:       → {condition}")
        term_print(f"Airspeed:        {V:.1f} m/s")
        term_print(f"Altitude:        {h:.0f} m")
        term_print(f"Lift:            {Lift:,.0f} N")
        term_print(f"Drag:            {Drag:,.0f} N")
        term_print(f"Load factor n:   {n:.2f}")
        term_print(f"L/D ratio:       {L_D:.1f}")
        term_print(f"Thrust avail:    {Thrust_available:,.0f} N")
        term_print("══════════════════════════════════════")

    except tk.TclError:
        warn("ERROR: Invalid number in one or more fields")
    except Exception as e:
        warn(f"CRASH: {type(e).__name__}: {e}")
